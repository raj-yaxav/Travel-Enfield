...
function extractFooterExciting(document) {
  return document.match(/<section\b[^>]*\bclass=["'][^"']*\bfooter-exciting\b[^"']*["'][^>]*>[\s\S]*?<\/section>/i)?.[0] || '';
}
...
const sharedFooterExciting = extractFooterExciting(sourceDocuments.home);
...
export function readLegacyBody(source) {
  ...
  if (source === 'app' && sharedFooter) {
    body = body.replace(/<section\b[^>]*\bclass=["'][^"']*\bfooter-exciting\b[^"']*["'][^>]*>[\s\S]*?<\/section>/i, '');
    body = body.replace(/<footer\b[^>]*>[\s\S]*?<\/footer>/gi, '');
    body += sharedFooterExciting + sharedFooter;
  }
